...
def select_prompt_type(
    caption_type: str, length: str | int, captions: Dict[str, List[str]]
):
    # Build prompt
...
def process_images(images: List[PIL.Image.Image]) -> List[torch.Tensor]:
    def process_image(source_image: PIL.Image.Image) -> torch.Tensor:
        target_image = (
            source_image.resize((384, 384), PIL.Image.LANCZOS)
            if source_image.width != 384 or source_image.height != 384
            else source_image
        )
        return transforms.functional.normalize(
            transforms.functional.pil_to_tensor(target_image).unsqueeze(0) / 255.0,
            mean=[0.5, 0.5, 0.5],
            std=[0.5, 0.5, 0.5],
        ).to(dtype=torch.float16, device="cuda")

    return [
        process_image(image) for image in tqdm.tqdm(images, desc="Tensoring images")
    ]
...
def generate_captions(
    ...
    with torch.no_grad():
        convo_embeds = text_model.model.embed_tokens(
            convo_tokens.unsqueeze(0).to("cuda")
        )

    resulting_captions: List[str] = []
    image_chunks = (
        break_list_into_chunks(images, batch_size) if batch_size > 1 else images
    )

    for chunk in tqdm.tqdm(image_chunks, desc="Processing images"):
        pixel_values = torch.stack(
            process_images(chunk if batch_size > 1 else [chunk])
        ).squeeze(
            1
        )  # torch.stack(process_images(images)).squeeze() if len(images) > 1 else process_images(images)
            ...
            embeds.append(
                torch.cat(
                    [
                        ...
                        convo_embeds[
                            :, preamble_len:
                        ],  # The prompt and anything after it
                    ],
                    dim=1,
                ).to("cuda")
            )
        ...
        with torch.no_grad():
            generate_ids = text_model.generate(
                (
                    ...
                    if len(tokens) > 1
                    else torch.stack(tokens).squeeze(0)
                ),
                inputs_embeds=(
                    torch.stack(embeds).squeeze()
                    if len(embeds) > 1
                    else torch.stack(embeds).squeeze(0)
                ),
        ...
        trim_generate_ids = torch.stack(
            [
                generate_ids[n : n + 1, tokens[n].shape[1] :]
                for n in range(0, len(generate_ids))
            ]
        ).squeeze(1)
        batch_captions: List[str] = tokenizer.batch_decode(
            trim_generate_ids,
